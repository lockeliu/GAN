...
class Trainer(object):
    ...
    def train(self):
        ...
        for batch_num, data in enumerate(self.training_loader):
            ...
            D_real_loss = self.BCE_loss( self.D_model( data ), y_real_ )

            noise = torch.randn( ( batch_size, 100 ) )
            D_fake_loss = self.BCE_loss( self.D_model( self.G_model( noise) ), y_fake_ )
            ...
            noise = torch.randn( ( batch_size, 100 ) )
            G_loss = self.BCE_loss( self.D_model( self.G_model( noise ) ), y_real_ );
